...
try
{
	...
	{ //infinite loop
        ...
        $docID = $DB->PUBLISHING_DOCUMENT_GET_PUBLISHING_ID($collectionName); //look for abandoned job
      
		//Check if there are any abandoned jobs in first collection
        if ($docID == null) 
		{
			...
			if($docID == null) //THE FIRST COLLECTION HAS NO ITEMS MOVE ON TO 2nd COLLECTION
			{
				  echo "\nThere is no items in the " . $collectionName . " Queue.\n";   
				//Hardcoding the collections, this should be automated later
				if($collectionName == "bluchermaps")
				{
					//Switch to green
					$DB->SWITCH_DB("greenmaps");	
					$collectionName = "greenmaps";
					echo "\nSwitching DB to : " . $collectionName . ".\n"; 
					$collection = $DB->GET_COLLECTION_INFO($collectionName);
					$TDLCommunityID = $collection["TDLcommunityID"];
					$TDLCollectionID = $collection["TDLcollectionID"];
					
				}
				else if($collectionName == "greenmaps")
				{
					//Switch to Fieldbooks
					$DB->SWITCH_DB("blucherfieldbook");
					$collectionName = "blucherfieldbook";
					echo "\nSwitching DB to : " . $collectionName . ".\n"; 
					$collection = $DB->GET_COLLECTION_INFO($collectionName);
					$TDLCommunityID = $collection["TDLcommunityID"];
					$TDLCollectionID = $collection["TDLcollectionID"];
				...
				{
					//If its fieldbooks switch to bluchermaps
					$DB->SWITCH_DB("bluchermaps");
					$collectionName = "bluchermaps";
					echo "\nSwitching DB to : " . $collectionName . ".\n"; 
					$collection = $DB->GET_COLLECTION_INFO($collectionName);
					$TDLCommunityID = $collection["TDLcommunityID"];
					$TDLCollectionID = $collection["TDLcollectionID"];
				    ...
				    $docID = $DB->PUBLISHING_DOCUMENT_GET_PUBLISHING_ID($collectionName); 
					if ($docID == null) 
					{
						 $docID = $DB->PUBLISHING_DOCUMENT_GET_NEXT_IN_QUEUE_ID($collectionName);
						 if($docID == null)// second collection has no items
						 {
							echo "\nThere is no items in the " . $collectionName . " Queue.\n";   
								 //Hardcoding the collections, this should be automated later
							if($collectionName == "bluchermaps")
							...
							{
								 $docID = $DB->PUBLISHING_DOCUMENT_GET_NEXT_IN_QUEUE_ID($collectionName);
								 if($docID == null)// third collection has no items
								 {
								    echo "\nThere is no items in the " . $collectionName . " Queue.\n";  
										 //Hardcoding the collections, this should be automated later
									if($collectionName == "bluchermaps")
									...
									if ($docID == null) 
									{
										...
										echo "\nThere is no items in any of the DB's. Sleep for 30 seconds.";              
										sleep(30);
										//Reset everything
										$DB = null;
										$DS = null;
        ...
        if($docID != null)		//begin if[1]
        {
            ...
            if($debug) 
			{
				...
				{
					 $fbCount = $FB->COUNT_FIELDBOOK_BOOKS($collectionName, $doc["BookTitle"]);					
					 print_r($Schema->convertSchema($collection["templateID"],$doc,true,$fbCount[0][0]));
				}
				else
				{
            ...
            switch($doc_dspace["dspacePublished"])
            {
                case 2:
					...
					{
						$fbCount = $FB->COUNT_FIELDBOOK_BOOKS($collectionName, $doc["BookTitle"]);
						$convertedSchema = $Schema->convertSchema($collection['templateID'],$doc, true, $fbCount[0][0]);
					}
					else
					{
						$convertedSchema = $Schema->convertSchema($collection['templateID'],$doc);
					}
			...
			if($collectionName == "blucherfieldbook")
			{
				...
				else
				{
					...
					else
					{
						...
						$publishedBitstreams = $DS->TDL_GET_ITEM_BITSTREAMS($dspaceID);
						...
						foreach($publishedBitstreams as $pB)
						{
							...
							if($pB->name == $iB->name) 
							{
								...
								if($pB->checkSum != $iB->md5_checksum) //compare checksum
								{
									...
									$http_retval = $DS->TDL_DELETE_BITSTREAM($pB->uuid,"application/json");//TDL Version 5 :: $pB->id  //TDL Version 6 :: $pB->id
									fwrite($errorLog,date(DATE_RFC2822) . ": http_retval3" . $http_retval["http_code"] . " \r\n");
									if($http_retval["http_code"] == "200")
										$published = false;
									else fwrite($logfile, date(DATE_RFC2822) . ": " . $iB->type . " failed to delete bitstreams from TDL " . $pB->name .  ".... HTTP CODE:" . $http_retval["http_code"] . "\r\n");
								}
 ...
 *******************************************/
	...
	function generateProvenance($dspaceID)
	{
		...
		$item = json_decode($TDL->TDL_GET_ITEM_METADATA($dspaceID));
		$bitstreams = json_decode($TDL->TDL_GET_ITEM_BITSTREAMS($dspaceID));
		$datetime = null;
