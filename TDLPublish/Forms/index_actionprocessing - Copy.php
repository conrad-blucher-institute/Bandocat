...
//var_dump($TEST);
	...
	switch($_POST['action'])
	{
		...
		case "update":
			...
			$publicBitstreams = $TDL->TDL_GET_BITSTREAMS($dspaceID);
			$internalBitstreams = array();
			//Assume each item has a back and is rectified
			$hasBack = true;
			...
			if(preg_match('/back/',$doc["FileName"]))
			{
				...
				CREATE_TICKET($collection["collectionID"], $doc["FileName"] . " :: FRONT CONTAINS (BACK) :: ", "Auto Update: FAILED", json_encode($LIBARRAY),$session);
				fwrite($Log,date(DATE_RFC2822) . ": " . "\r\nFAILED: " . $doc["FileName"] ."  \r\nTYPE: FRONT CONTAINS (BACK)   \r\nLOCATION:INTERNAL" ." \r\n\r\n");
				continue;
			}
			//Verify if the item has a back
			...
			else
			{
				...
				{
					$LIBARRAY = array("libraryIndex"=>$doc["LibraryIndex"]);			
					CREATE_TICKET($collection["collectionID"], $doc["FileNameBack"] . " :: BACK DOES NOT CONTAIN (BACK) :: ", "Auto Update: FAILED", json_encode($LIBARRAY),$session);
					fwrite($Log,date(DATE_RFC2822) . ": " . "\r\nFAILED: " . $doc["FileNameBack"] ."  \r\nTYPE: BACK DOES NOT CONTAIN (BACK)   \r\nLOCATION:INTERNAL" ." \r\n\r\n");
					continue;
				}
			}
			//Verify if the item has been rectified || 1 = Rectified and file exists || 0 and 2 = file does not exist
			...
			else
			{
				if(preg_match('/back/',$doc["georecFrontDirKMZ"]) || preg_match('/back/',$doc["georecFrontDirGeoTIFF"]))
				{
					//ISSUE
					$LIBARRAY = array("libraryIndex"=>$doc["LibraryIndex"]);			
					CREATE_TICKET($collection["collectionID"], $doc["georecFrontDirKMZ"] . " :: FRONT CONTAINS (BACK) :: ", "Auto Update: FAILED", json_encode($LIBARRAY),$session);
			...
			else
			{
				if(!preg_match('/back/',$doc["georecBackDirKMZ"]) || !preg_match('/back/',$doc["georecBackDirGeoTIFF"]))
				{
					//ISSUE
					$LIBARRAY = array("libraryIndex"=>$doc["LibraryIndex"]);			
					CREATE_TICKET($collection["collectionID"], $doc["georecBackDirKMZ"] . " :: BACK DOES NOT CONTAIN (BACK) :: ", "Auto Update: FAILED", json_encode($LIBARRAY),$session);
		  ...
		  //  print_r($internalBitstreams);
												...
												if($pbstream["sizeBytes"] == 0 || $publicbit->checkSum->value != $internalbit->md5_checksum )
												{
													for($i = 0; $i < 3; $i++)
													{
														...
														//The bitstreams filesize is 0 OR the MD5 checksum does not match	
														 ...
														 {
															$LIBARRAY = array("libraryIndex"=>$doc["LibraryIndex"]);			
															CREATE_TICKET($collection["collectionID"], $internalbit->path . " :: SIZE/CHECKSUM/PUT FAILURE :: ", "Auto Update: FAILED", json_encode($LIBARRAY),$session);
			...
			if($publicCount > count($internalBitstreams))
			{
				...
				foreach($publicBitstreams as $publicbit)
				{
					...
					{
						$LIBARRAY = array("libraryIndex"=>$doc["LibraryIndex"]);					
						CREATE_TICKET($collection["collectionID"], $publicbit->uuid . " :: TDL COUNT DISCREPANCY, BITSTREAM DELETE FAILED :: ", "Auto Update: FAILED", json_encode($LIBARRAY),$session);
			...
			for($i = 0; $i < 3; $i++)
			{
				foreach($internalBitstreams as $internalbit)
				{
					...
					if($internalbit->type == "frontJPG" && $tdlHasFrontJPG == false && $FRONTJPG == false)
					{						
						...
						if($http_retval["http_code"] == "200")
						{
							...
							{
								$LIBARRAY = array("libraryIndex"=>$doc["LibraryIndex"]);
								CREATE_TICKET($collection["collectionID"], $internalbit->path . " :: SIZE FAILURE :: ", "Auto Update: FAILED", json_encode($LIBARRAY),$session);
						...
						if($i == 2 && $FRONTJPG == false)
						{
							$LIBARRAY = array("libraryIndex"=>$doc["LibraryIndex"]);				
							CREATE_TICKET($collection["collectionID"], $internalbit->path . " :: POST FAILURE :: " . $http_retval["http_code"], "Auto Update: FAILED", json_encode($LIBARRAY),$session);
							fwrite($Log,date(DATE_RFC2822) . ": " . "\r\nFAILED: " . $internalbit->name ."  \r\nTYPE: UPLOAD FAILED .   \r\nLOCATION: https://tamucc-ir.tdl.org/handle/" . $dspaceDocInfo['dspaceURI']."  \r\n\r\n");
						}
			...
			$ret = $DS->TDL_PUT_ITEM_METADATA($dspaceID,$jsonfilename);
...
//WRITE A LOG
	...
	function CREATE_TICKET($COLLECTION, $DESCRIPTION, $SUBJECT, $LIBINDEX, $SESSION)
	{
		$DB = new DBHelper();
		$result = $DB->SP_TICKET_INSERT($SUBJECT, $SESSION->getUserID(),$COLLECTION,$DESCRIPTION, $LIBINDEX);
		print_r($result);
	}
?>